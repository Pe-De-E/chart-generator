<template>
  <div class="routemap-generator-layout">
    ...
    <!-- Main Content Area -->
    <div class="routemap-generator-content" :class="{ 'nav-collapsed': stepNavCollapsed }">
...
const workflowSteps = [
  ...
  },
  {
    ...
    description: 'Route + Hoehenprofil generieren',
...
const animationConfig = ref<RouteMapAnimationConfig>({ ...DEFAULT_ROUTEMAP_ANIMATION_CONFIG })
...
function handleFileUpload(files: File | File[] | null) {
  ...
  reader.onload = (e) => {
    ...
    // Store route points with lat/lon
...
function resetWizard() {
  ...
  routePoints.value = []
  resetData()
  animationConfig.value = { ...DEFAULT_ROUTEMAP_ANIMATION_CONFIG }
  chartTitle.value = 'Route Map'
}
...
.routemap-generator-content.nav-collapsed {
  margin-left: 72px;
}

.routemap-generator-content :deep(.v-window) {
  height: 100%;
}

.routemap-generator-content :deep(.v-window__container) {
  height: 100%;
}

.routemap-generator-content :deep(.v-window-item) {
  height: 100%;
}

.routemap-generator-content :deep(.v-card) {
