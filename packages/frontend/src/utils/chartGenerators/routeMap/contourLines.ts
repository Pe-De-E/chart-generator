...
// (the project requires Cross-Origin-Embedder-Policy: require-corp for FFmpeg)
const TILE_URL = '/terrain-tiles'

/**
 * Convert lon/lat to slippy map tile coordinates at a given zoom level.
 */
...
/**
 * Determine all tiles needed to cover the given bounds at a zoom level.
 */
function getTilesForBounds(bounds: RouteBounds, zoom: number): TileCoord[] {
...
/**
 * Choose zoom level based on route extent for appropriate contour resolution.
 */
function chooseZoom(bounds: RouteBounds): number {
  const latSpan = bounds.maxLat - bounds.minLat
  const lonSpan = bounds.maxLon - bounds.minLon
  const extent = Math.max(latSpan, lonSpan)
  if (extent < 0.3) return 12   // ~30m resolution, tight route
  if (extent < 1.0) return 11   // ~60m resolution
  return 10                      // ~90m resolution, wide route
}

// ── Tile Fetching & Decoding ─────────────────────────────────────────────────

// Cache fetched tile image data to avoid re-downloading
const tileCache = new Map<string, Float32Array>()

/**
 * Fetch a terrain tile PNG, decode Terrarium RGB → elevation.
 * Uses offscreen Canvas to read pixel data.
 */
...
/**
 * Fetch all tiles for a bounding box and stitch into a single elevation grid.
 */
async function fetchElevationGrid(bounds: RouteBounds, zoom: number): Promise<ElevationGrid> {
  const tiles = getTilesForBounds(bounds, zoom)

  if (tiles.length === 0) throw new Error('No tiles found for bounds')

  // Fetch all tiles in parallel
  const tileResults = await Promise.all(tiles.map(async t => ({
    tile: t,
    decoded: await fetchAndDecodeTile(t),
  })))

  // Compute the combined grid bounds and dimensions
  ...
  const tileSize = 256
  const gridW = tilesWide * tileSize
  const gridH = tilesHigh * tileSize

  const grid = new Float32Array(gridW * gridH)

  // Stitch tiles into the combined grid
  for (const { tile, decoded } of tileResults) {
    ...
    for (let row = 0; row < tileSize; row++) {
      ...
      const dstStart = (offsetY + row) * gridW + offsetX
      grid.set(decoded.data.subarray(srcStart, srcStart + tileSize), dstStart)
    }
  }
